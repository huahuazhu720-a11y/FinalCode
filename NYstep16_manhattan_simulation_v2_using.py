"""
history verison of simulation, incase needed
"""
import sympy as sp
import numpy as np
import pandas as pd
...
if __name__ == "__main__":
    ...
    max_Va_VE_travelor=df['VE_travelor'].max()
    max_Va_VE_fixed_flow_without_Manhattan=df['fixed_flow_without_Manhattan'].max()
 
    scale_factor_max_VE = max_Va_VE_fixed_flow_without_Manhattan / 11000
    df['VE_travelor']=df['VE_travelor']/12
    # df['fixed_flow_without_Manhattan']=df['fixed_flow_without_Manhattan']/scale_factor
    df['fixed_flow_without_Manhattan']=df['fixed_flow_without_Manhattan']/12
    max_Va_PT_travelor=df['PT_travelor'].max()
    scale_factor_max_PT = max_Va_PT_travelor / 11000
    df['PT_travelor']=df['PT_travelor']/12
    df['fixed_flow_without_Manhattan_PT']=df['fixed_flow_without_Manhattan_PT']/12
    ...
    od_demand_list = np.exp(od_demand_list)*2
    ...
    for i in range(500):       
        ...
        if VE_travel_time_CHANGE <=threshold :
            ...
            df['PT_traffic_flow']=df['PT_traffic_flow']
            df['VE_traffic_flow']=df['VE_traffic_flow']
            df["VE_cost_linK"]=VE_value_time* ((df['VE_traffic_flow']+ df['fixed_flow_without_Manhattan'])*df['kappa'] +df['VE_FreeF_travel_time'])+df['VE_fixed_cost']+(1+df['VE_traffic_flow']+df['fixed_flow_without_Manhattan'])*np.log((1+df['VE_traffic_flow'])+df['fixed_flow_without_Manhattan'])-(df['VE_traffic_flow']+df['fixed_flow_without_Manhattan'])
            df["PT_cost_linK"]=PT_value_time* (df['PT_FreeF_travel_time'])+df['PT_fixed_cost']+(1+df['PT_traffic_flow']+df['fixed_flow_without_Manhattan_PT'])*np.log((1+df['PT_traffic_flow']+df['fixed_flow_without_Manhattan_PT']))-(df['PT_traffic_flow']+df['fixed_flow_without_Manhattan_PT'])
            ...
            print(f"all links total flow:{df['VE_traffic_flow'].sum()+df['PT_traffic_flow'].sum()}")
