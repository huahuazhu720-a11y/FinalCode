...
def run():
    ...
    # edgedata使用simulate计算出来的
    netGraph = pd.read_csv('price0_edgedata.csv')
    G = nx.DiGraph()    
    ...
    #     if data['transit_travelor_Manhattan']>max_congestion:
    #         max_congestion=data['transit_travelor_Manhattan']
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list(
        'traffic_cmap', ['green', 'red', 'darkred'], N=100
    )
    ...
    for i, row in districts.iterrows():
        ...
        for i in adjacent_districts[zone_id]: 
            try:
                ...
                pass
            flow=Net_work[zone_id][i].get("transit_travelor_Manhattan", False) # 跳过没有的
            if not flow: # 注意：：flow是0的，也会跳过
                continue

            try:
                edge_data = Net_work[zone_id][i].get("edge_indices", None)
                congestion = G[zone_id][i].get("VE_traffic_flow", None)
    ...
    if roude_graph and not roude_coler:
        gpd.GeoSeries(all_routes).plot(ax=ax, color='black', linewidth=1.5)
    if roude_graph and roude_coler:
        gpd.GeoSeries(all_routes).plot(ax=ax, color=edge_colors, linewidth=1.5)
    #================上面代码是把node的连接用路网绘制========================
